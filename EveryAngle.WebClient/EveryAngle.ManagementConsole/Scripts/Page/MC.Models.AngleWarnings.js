	...
	function AngleWarnings() {
		...
		self.SolutionChanged = function (obj) {
			...
			if (!action) {
				...
				// replace action
				var actionText = obj.children(':selected').text();
				var hintText = '';
		...
		self.CheckExecutionAngleWarning = function (uri) {
			...
			var getFailMessage = function (xhr, status, error) {
				var message = MC.ajax.getErrorMessage(xhr, null, error);
				return '<li class="fail">' + jQuery(message).filter('p').text() + '</li>';
			};
		...
		var createConfirmMessage = function (selectAction, dataItem, treelist) {
			var solutionOption = selectAction.find('option:selected').val();
		...
		self.ClearUnusedActionArgumentParameter = function (actionArgument, actionArgumentParameter, target) { //deleteds, target) {
					...
					if (action.DataItem && action.DataItem.hasChildren) {
						$.when(getTargetData(action.DataItem, self.GetAllThirdLevelUrl, treelist))
							.done(function (response) {
								...
								actionArgument = self.ClearUnusedActionArgumentParameter(actionArgument, actionArgumentParameter, response);
