        ...
        public void Save(MiniProfiler profiler)
        {
            ...
            if (HttpContext.Current.Response.Filter is OutputFilterStream || HttpContext.Current.Response.Filter.GetType().ToString().Equals("System.Web.HttpResponseStreamFilterSink"))
            {
                ...
                try
                {
                    ...
                    {
                        contentLength = Encoding.Default.GetBytes(((OutputFilterStream)HttpContext.Current.Response.Filter).ReadStream()).Length;
                    }
                    else if (
                ...
                {
                    EveryAngle.Logging.Log.SendInfo("Cannot read the content from stream: " + exception.Message);
                }
                Log4NetLogger.Info(string.Format("{0}\t{1}\t{2}\t{3}\t{4}\t{5}", HttpContext.Current.Request.HttpMethod, profiler.Root.Name, HttpContext.Current.Response.StatusCode, HttpContext.Current.Request.ContentLength, contentLength, profiler.DurationMilliseconds));
            }
        }
