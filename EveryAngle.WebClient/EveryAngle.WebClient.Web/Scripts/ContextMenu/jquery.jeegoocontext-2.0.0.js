...
(function($){
    ...
    var _resetMenu = function(){
		...
		$(document).off('.jeegoocontext');
		// Unbind resize event bound to the window.
		$(window).off('resize.jeegoocontext');
    };
	...
	$.fn.jeegoocontext = function(id, options){
	    ...
	    $('#' + id).on('mouseover.jeegoocontext', 'li', function (e) {
            ...
            e.stopPropagation(); 
	    }).on('click.jeegoocontext', 'li', function(e){
        ...
        return $(_menus[id].wrapper).on(eventType, _menus[id].allContext, function (e) {
            ...
            if(_menus[id].openBelowContext)
            {
                $(window).on('resize.jeegoocontext', function(){
			...
			$(document).on('mouseover.jeegoocontext', function(e){ 
			...
			}).on('click.jeegoocontext', _globalHide);

			if(_menus[id].keyboard)
			{
			    $(document).on('keydown.jeegoocontext', function (e) {
			        switch(e.which)
			        {
			            case 38: //keyup
			    ...
			    }).on('keyup.jeegoocontext', function(e){
			        clearInterval(_global.keyUpDown);
			        _global.keyUpDownStop = false;
			    });
			}
