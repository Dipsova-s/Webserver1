var dashboardDetailBodyHtmlTemplate = function () {
    return [
        '<ul class="popupTabMenu" id="DashboardTabs">',
            ...
            '<li class="fieldsfilters" onclick="dashboardDetailsHandler.TabClick(dashboardDetailsHandler.TAB.FIELDSFILTERS);"><a>' + Localization.DashboardFilters_Title + '</a></li>',
        ...
        '<div class="popupTabPanel" id="DashboardArea">',
            '<div class="generalArea">',
                '<h2 data-bind="text: Localization.General"></h2>',
                '<div class="row">',
                    '<div class="field" data-bind="text: Localization.Model"></div>',
                    '<div class="input"><div class="k-dropdown" id="SystemModels"></div></div>',
                '</div>',
                '<div class="row">',
                    '<div class="field" data-bind="text: Localization.BusinessProcesses"></div>',
                ...
                '<div class="row">',
                    ...
                    '<div class="input">',
                        '<input id="AngleId" type="text" required data-required-msg="Required" class="eaText eaTextSize40" data-bind="value: Model.Data().id, enable: Model.CanUpdateDashboard(\'id\')" maxlength="50" />',
                    '</div>',
                '</div>',
                '<!-- /ko -->',
            ...
            '<h2>' + Localization.DashboardFilters_Title + '</h2>',
                '<div class="DefInfo">',
                    '<label>' + Localization.DashboardFilters_Description + '</label>',
                    '<div id="FieldsFiltersWrapper"></div>',
                '</div>',
                '<div class="filterButtonWrapper">',
                    '<a id="DashboardAddfilter" class="btn btn-ghost btnAddFilter" data-bind="css: $root.Model.CanUpdateDashboard(\'query_difinitions\') ? \'\' : \'disabled\', click: $root.ShowAddFilterPopup"><span data-bind="text: Localization.AddFilter">' + Localization.AddFilter + '</span></a>',
                '</div>',
            '</div>',
                                ...
                                '<div class="FilterDetail Hide">',
                                    ...
                                    '<div class="widgetDetailRow widgetDetailRowAngle">',
                                        ...
                                        '<div class="widgetDetailValue">',
                                            '<div class="displayNameContainer" data-bind="html: def.angle_name"></div>',
                                        '</div>',
                                    '</div>',
                                        ...
                                        '<div class="widgetDetailValue FilterCriteriaPlaceHolder">',
                                            '<div class="DisplayItemList" data-bind="foreach: { data: def.displays, as: \'display\' }">',
                                                '<div class="ItemList" data-bind="click: $root.DefinitionChangeWidget.bind(display, display, $root.DefinitionCanSelectDisplay(display)), css: { disabled: !$root.DefinitionCanSelectDisplay(display), ItemListSelected: JSON.parse(def.selected()).id === display.id }">',
                                                    '<div class="displayNameContainer" data-bind="html: display.name_html"></div>',
