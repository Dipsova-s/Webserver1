...
// 2011 Adapted for EA by Bart van Dongen
...
function QueryServiceStatus(hService: Longword; var lpServiceStatus: _SERVICE_STATUS): Longword;
...
function ChangeServiceConfig2(hService: Longword; dwInfoLevel: Longword; lpInfo: _SERVICE_DESCRIPTION): BOOL;
  external 'ChangeServiceConfig2{#AW}@advapi32.dll stdcall';
  ...
  try
    ...
    try
      ...
      begin
        ...
        log(Format('[i]SimpleCreateService: Created:"%s" for "%s"', [AServiceName, AFileName]));
      Result := True;
    finally
      if ServiceHandle <> 0 then
...
begin
  Result := 0;
  try
    SCMHandle := OpenSCManager('', '', SC_MANAGER_QUERY_ACCESS);
    if SCMHandle = 0 then
      RaiseException('OpenSCManager: ' + AService + ' ' + SysErrorMessage(DLLGetLastError));
    try
      ServiceHandle := OpenService(SCMHandle, AService, SERVICE_QUERY_STATUS_ACCESS);
      if ServiceHandle = 0 then
      begin
        Error := DLLGetLastError;
        if Error = ERROR_SERVICE_DOES_NOT_EXIST then
      ...
      try
	      if QueryServiceStatus(ServiceHandle, ServiceStatus) = 0 then
        begin
          Error := DLLGetLastError;
          RaiseException('QueryServiceStatus: ' + AService + ' ' + SysErrorMessage(Error));
        end;
...
function ServiceIsIdle(AService: string): Boolean;
var
  Status : Longword;
begin
  Status := SimpleQueryService(AService);
  Result := 
        ( Status = SERVICE_STOPPED )
     or ( Status = ERROR_SERVICE_DOES_NOT_EXIST );
  log(IntToStr(Status) + ' :'+ aService);
end;

procedure SimpleChangeServiceDescription(aService, aDescription: string);
var
  SCMHandle: Longword;
  ServiceHandle: Longword;
...
begin
  try
    SCMHandle := OpenSCManager('', '', STANDARD_RIGHTS_REQUIRED);
    if SCMHandle = 0 then
      RaiseException('SimpleChangeServiceConfig@OpenSCManager: ' + aService + ' ' + 
        SysErrorMessage(DLLGetLastError));
    try
      ServiceHandle := OpenService(SCMHandle, aService, SERVICE_CHANGE_CONFIG);
      if ServiceHandle = 0 then
        RaiseException('SimpleChangeServiceConfig@OpenService: ' + aService + ' ' + 
          SysErrorMessage(DLLGetLastError));
      try
        ServiceDescription.lpDescription := aDescription;
