...
Verify the Datastore is deleted successfully
    [Arguments]     ${datastorePlugin}      ${datastoreName}
    Click on Delete in action drop down by Datastore name   ${datastoreName}
...
Verify the edited value is displayed in SQL Datastore
    ...
    Click on Edit in action drop down by Datastore name     ${editedDatastoreName}
    Verify the field values for Datastore Name  ${editedDatastoreName}     none
    Verify the field values for SQL Datastore in Connection Settings    nl-bangalore1\SQLEXPRESS  sa  ea  2019sp2  check
    Verify the field values for SQL Datastore in Data Settings    6  Long name  Id  5  {anglename:edited}  99  uncheck