...
${btnDeleteAction}      css=#DataStoresGridContainer tbody tr .btnGroupInner .btnDelete
...
Click on Save button in DataStore
    ...
    Wait Until Element Exist And Visible  //div[@id='DataStoresGridContainer']//tr/td[text()='${datastorePlugin}']/parent::tr/td[text()='${datastorePlugin}']
...
Verify the DataStoresGrid is filtered with Text
    [Arguments]     ${filterText}
    @{rowList}  Get Grid Column Texts  css=#DataStoresGrid .k-grid-content tr  1
    ${rowCount}  Get Element Count  css=#DataStoresGrid .k-grid-content tr
...
Click on Delete in action drop down by Datastore name
    [Arguments]    ${name}
    Wait Until Page Contains Element    ${trRowDatastoreGrid}:contains(${name}) ${btnEditAction}
    Scroll Horizontal  ${scrollBarHorizontalGrid}  2000
    Scroll Vertical To Element    ${scrollBarGrid}    ${trRowDatastoreGrid}:contains(${name})
    Click Element At Coordinates       ${trRowDatastoreGrid}:contains(${name}) ${btnEditAction}         10    0
    Wait Until Page Contains Element    ${btnDeleteAction}
    Click Element   ${btnDeleteAction}
